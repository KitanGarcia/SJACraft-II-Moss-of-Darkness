...
CAIPlayer::CAIPlayer(std::shared_ptr< CPlayerData > playerdata, int downsample, std::string luaFile){
    ...
    DLuaFile = new char[luaFile.length() + 1];
    strcpy(DLuaFile, luaFile.c_str());
}
//Lua Getters
 ...
 */
int CAIPlayer::GetNumberOfGoldMines(lua_State *L){
    CAIPlayer* aiptr = (CAIPlayer*)lua_topointer(L, -1);
    int count = 0;
    for( auto asset : aiptr->DPlayerData->ActualMap()->Assets())
    {
        if(asset->Type() == EAssetType::GoldMine)
        count++;
    }
...
int CAIPlayer::GetNearestTileOfType(lua_State *L){
    ...
    CTerrainMap::ETileType tileType = TileNameToType(lua_tostring(L, -1));
...
/**
 * Gets the nearest asset to a location
 *
 * ---Parameters and returns are documented as Lua Side
 *
 * @param[in] AIPointer Pointer to the CAIPlayer to reference
 *
 * @param[in] TargetType The type of asset to search for
 * @param[in] X,Y The location to search from
 *
 * @return The AssetID of the nearest asset of the given type
 */
int CAIPlayer::GetNearestAssetFromOfType(lua_State *L){
    CAIPlayer* aiptr = (CAIPlayer*)lua_topointer(L, -4);
    EAssetType searchType = ResolveAssetTypeFromName( aiptr, lua_tostring(L,-3));
    CPixelPosition searchPos; 
    searchPos.SetFromTile( CTilePosition(lua_tointeger(L,-2), lua_tointeger(L,-1)));
    auto SearchedAsset = aiptr->DPlayerData->FindNearestAsset(searchPos, searchType);
    lua_pushnumber(L, SearchedAsset->AssetID());
    return 1;
}
/**
...
int CAIPlayer::FindAssetPlacementWithConstraints(lua_State *L){
    ...
    EAssetType assetType = ResolveAssetTypeFromName( aiptr, lua_tostring(L,-2));
...
int CAIPlayer::GetAssetAction(lua_State *L){
    int assetID = lua_tointeger(L, -1);
    std::shared_ptr<CPlayerAsset> asset = FindAssetObj(assetID);
    // printf("%d, %s",asset->Action(), ActionTypeToName(asset->Action()).c_str());
    lua_pushstring(L, ActionTypeToName(asset->Action()).c_str());
}
...
/**
*   Gets the assetID and X,Y coordinate position of the ONE and ONLY Townhall.
*   This function only works properly assuming there is no more than one town hall
...
*/
int CAIPlayer::GetTownHallPos(lua_State *L) {
    ...
    for(auto WeakAsset : aiptr->DPlayerData->Assets()){
        if(auto Asset = WeakAsset.lock()){
            if(Asset->HasCapability(EAssetCapabilityType::BuildPeasant)){
                TownHallAsset = Asset;
                lua_pushnumber(L, TownHallAsset->AssetID());
...
/* Checks if a peasant is already building something
 * @param Asset - Expected to be an CPlayerAsset of type peasant.
 * 
 * @return true if it has the active capability to build some structure, else false.
 */
bool PeasantHasBuild(std::shared_ptr<CPlayerAsset> Asset)
{
    EAssetCapabilityType Buildable[6] =  { 
        EAssetCapabilityType::BuildFarm, 
    ...
    for( auto a : Buildable)
    {
        if(Asset->HasActiveCapability(a))
        {
            return true;
        }
    }
...
/**
 * find a interruptible buidler to do the building
 *
 * @param[in] aiptr A ptr to a struct containing AI player data
 * @param[in] buildingtype Enum value that dictates what kind of building is constructed
 *
 * @return the id of the builder if avalaible, -1 if none is avalible
 */
int CAIPlayer::GetBuilder(lua_State *L){
    std::shared_ptr<CPlayerAsset> BuilderAsset ;
    std::shared_ptr<CPlayerAsset> TownHallAsset;
    std::shared_ptr<CPlayerAsset> NearAsset;
    CAIPlayer* aiptr = (CAIPlayer*)lua_topointer(L, -3);
    EAssetCapabilityType BuildAction = ResolveAssetCapabilityFromName( lua_tostring(L,-2));

    EAssetType buildingType = ResolveAssetTypeFromName( aiptr, lua_tostring(L,-1));
    // printf("start: %d, %d", BuildAction, buildingType);
    bool AssetIsIdle = false;
    for(auto WeakAsset : aiptr->DPlayerData->Assets()){
        if(auto Asset = WeakAsset.lock()){
            if(Asset->HasCapability(BuildAction) && Asset->Interruptible() && !PeasantHasBuild(Asset) && !aiptr->DAssignedAssets[Asset->AssetID()]){
                if(!BuilderAsset || (!AssetIsIdle && (EAssetAction::None == Asset->Action()))){
                    BuilderAsset = Asset;
                    AssetIsIdle = EAssetAction::None == Asset->Action();
                //    printf("found an asset id:%d",Asset->AssetID());
                }
            }
            if(Asset->HasCapability(EAssetCapabilityType::BuildPeasant)){
                TownHallAsset = Asset;
            ...
            if(Asset->HasActiveCapability(BuildAction)){
                lua_pushnumber(L, -1);
                // printf("returned early 604");
                return 1;
            }
        }
    }
    if(BuilderAsset){
        // printf("Got Builder,  655, result:%d",BuilderAsset->AssetID());
...
//<<<<<<< HEAD
//    switch(buildingtype){
//        case EAssetType::Barracks:      BuildAction = EAssetCapabilityType::BuildBarracks;
//                                        break;
//        case EAssetType::LumberMill:    BuildAction = EAssetCapabilityType::BuildLumberMill;
//                                        break;
//        case EAssetType::Blacksmith:    BuildAction = EAssetCapabilityType::BuildBlacksmith;
//                                        break;
//        case EAssetType::Wall:    BuildAction = EAssetCapabilityType::BuildWall;
//                                        break;
//        default:                        BuildAction = EAssetCapabilityType::BuildFarm;
//                                        break;
//    }
//=======
...
int CAIPlayer::GetAllPlayerAssetsWithCapability(lua_State *L){
    CAIPlayer* aiptr = (CAIPlayer*)lua_topointer(L, -3);
    EAssetCapabilityType activeCapability = ResolveAssetCapabilityFromName(lua_tostring(L,-2));
    EAssetCapabilityType capability = ResolveAssetCapabilityFromName(lua_tostring(L,-1));
    ...
    for(auto WeakAsset : Assets){
        if(auto Asset = WeakAsset.lock()){
            if(!Asset->HasActiveCapability(activeCapability) && Asset->HasCapability(capability)){
                lua_pushnumber(L, Asset->AssetID());
                lua_rawseti(L, -2, index++);
                lua_pushnumber(L, Asset->TilePositionX());
                lua_rawseti(L, -2, index++);
...
int CAIPlayer::GetAllInteruptablePlayerAssetsWithCapability(lua_State *L){
    CAIPlayer* aiptr = (CAIPlayer*)lua_topointer(L, -2);
    EAssetCapabilityType capability = ResolveAssetCapabilityFromName(lua_tostring(L,-1));
...
/**
*   Gets the assetID and X,Y coordinate position of ALL assets of the desired type.
*   @param aiptr - pointer to the aiplayer
...
int CAIPlayer::GetLocationsOfAllAssetsOfType(lua_State* L){
    ...
    lua_newtable(L);
    int index = 1;
    auto Assets = aiptr->DPlayerData->ActualMap()->Assets();
...
*   @return lua_table - containing all assetIDs, and asset possition (X,Y)
*/
int CAIPlayer::GetLocationsOfAssetsOfType(lua_State* L){
    CAIPlayer* aiptr = (CAIPlayer*)lua_topointer(L, -2);
    EAssetType assetType = ResolveAssetTypeFromName(aiptr, lua_tostring(L,-1));
    bool found =false;
    lua_newtable(L);
    ...
    auto Assets = aiptr->DPlayerData->Assets();
    for(auto weakAsset : Assets){
        if(auto asset = weakAsset.lock() ) // needs a seperate line to deduce auto
        if( assetType == asset->Type()){
            found = true;
            lua_pushnumber(L, asset->AssetID());
            lua_rawseti(L, -2, index++);
...
/**
*   Gets the assetID and X,Y coordinate position of idle assets of the desired type.
*   @param aiptr - pointer to the aiplayer
*   @param  assetType - the asset that you want to add
*   @return lua_table - containing all assetIDs, and asset possition (X,Y)
*/
int CAIPlayer::GetLocationsOfIdleAssetsOfType(lua_State* L){
    CAIPlayer* aiptr = (CAIPlayer*)lua_topointer(L, -2);
    EAssetType assetType = ResolveAssetTypeFromName(aiptr, lua_tostring(L,-1));
    bool found =false;
    lua_newtable(L);
    int index = 1;
    auto Assets = aiptr->DPlayerData->IdleAssets();
    for(auto weakAsset : Assets){
        if(auto asset = weakAsset.lock() ) // needs a seperate line to deduce auto
        {
            if( assetType == asset->Type() && asset->Interruptible() ){
                found = true;
                // printf("%d, %d, %d", asset->AssetID(), asset->Interruptible(), asset->CurrentCommand().DAction);
                lua_pushnumber(L, asset->AssetID());
                lua_rawseti(L, -2, index++);
                lua_pushnumber(L, asset->TilePositionX());
                lua_rawseti(L, -2, index++);
...
/**
*   Gets the assetID and X,Y coordinate position of all building assets of the player.
*   @param aiptr - pointer to the aiplayer
*   @return lua_table - containing all assetIDs, and asset possition (X,Y)
*/
int CAIPlayer::GetAllPlayerBuildingAssets(lua_State* L){
    CAIPlayer* aiptr = (CAIPlayer*)lua_topointer(L, -1);
    ...
    for(auto WeakAsset : Assets){
        if(auto Asset = WeakAsset.lock()){
            if(Asset->Speed() == 0 && Asset->Type() != EAssetType::Wall){
                lua_pushnumber(L, Asset->AssetID());
                lua_rawseti(L, -2, index++);
                lua_pushnumber(L, Asset->TilePositionX());
                lua_rawseti(L, -2, index++);
...
int CAIPlayer::GetNearestEnemy(lua_State* L){
    ...
    auto NearestWall = aiptr->DPlayerData->FindNearestAsset(pos, EAssetType::Wall);

    if (!TargetAsset && !NearestWall){
        lua_pushnumber(L, -1);
    }
    else if (!TargetAsset && NearestWall){
        lua_pushnumber(L, NearestWall->AssetID());
    }
    else if (TargetAsset && !NearestWall){
        lua_pushnumber(L, TargetAsset->AssetID());
    }
    else{
        int distTarg = (TargetAsset->TilePositionX() - tileX) * (TargetAsset->TilePositionX() - tileX) + (TargetAsset->TilePositionY() - tileY) * (TargetAsset->TilePositionY() - tileY);
        int distWall = (NearestWall->TilePositionX() - tileX) * (NearestWall->TilePositionX() - tileX) + (NearestWall->TilePositionY() - tileY) * (NearestWall->TilePositionY() - tileY);
        if (distWall < distTarg)
            lua_pushnumber(L, NearestWall->AssetID());
        else
            lua_pushnumber(L, TargetAsset->AssetID());
    }
...
int CAIPlayer::CanInitiate(lua_State *L){
    ...
    std::string dummyStr = "";
    lua_pushboolean(L, playerCapability->CanInitiate(asset, aiptr->DPlayerData, dummyStr));
    return 1;
}
int CAIPlayer::GetLocationsOfAllEnemyTypes(lua_State* L){
  CAIPlayer* aiptr = (CAIPlayer*)lua_topointer(L, -2);
  EAssetType assetType = ResolveAssetTypeFromName(aiptr, lua_tostring(L,-1));
  EPlayerColor color = aiptr->DPlayerData->Color();

  lua_newtable(L);
  int index = 1;
  auto Assets = aiptr->DPlayerData->PlayerMap()->Assets();
  for(auto Asset : Assets){
    if(assetType == Asset->Type() && (Asset->Color() != color)){
      lua_pushnumber(L, Asset->AssetID());
      lua_rawseti(L, -2, index++);
      lua_pushnumber(L, Asset->TilePositionX());
      lua_rawseti(L, -2, index++);
...
int CAIPlayer::GetCanPlaceAssetOfSize(lua_State *L){
    CAIPlayer* aiptr = (CAIPlayer*)lua_topointer(L, -5);
    int assetSize = lua_tointeger(L, -4);
    int builderID = lua_tointeger(L, -3);
    std::shared_ptr<CPlayerAsset> builder = FindAssetObj(builderID);
    int tileX = lua_tointeger(L, -2);
    int tileY = lua_tointeger(L, -1);
    CTilePosition tilePos(tileX, tileY);
    lua_pushboolean(L, aiptr->DPlayerData->PlayerMap()->CanPlaceAsset(tilePos, assetSize, builder));
    return 1;
}

int CAIPlayer::GetToggle(lua_State *L){
    CAIPlayer* aiptr = (CAIPlayer*)lua_topointer(L, -2);
    int toggle = lua_tointeger(L, -1);
    lua_pushboolean(L, (aiptr->DToggles & (1 << toggle)) != 0);
    return 1;
}

int CAIPlayer::SwitchToggle(lua_State *L){
    CAIPlayer* aiptr = (CAIPlayer*)lua_topointer(L, -2);
    int toggle = lua_tointeger(L, -1);
    
    if ((aiptr->DToggles) & (1 << toggle))
        (aiptr->DToggles) &= (~(1 << toggle));
    else
        (aiptr->DToggles) |= (1 << toggle);
...
/*
Gets the Upgrade status of Armor, Weapon or Arrow
*/
int CAIPlayer::GetUpgradeStatus(lua_State *L){
    CAIPlayer* aiptr = (CAIPlayer*)lua_topointer(L, -2);
    EAssetCapabilityType capability = ResolveAssetCapabilityFromName(lua_tostring(L, -1));
    lua_pushboolean(L, aiptr->DPlayerData->HasUpgrade(capability));
	return 1;
}
...
void CAIPlayer::RegisterFunctions(lua_State *L){
    ...
    lua_register(L, "GetNumberOfGoldMines", GetNumberOfGoldMines);
    ...
    lua_register(L, "GetNearestAssetFromOfType", GetNearestAssetFromOfType);
    ...
    lua_register(L, "GetAssetAction", GetAssetAction);
    lua_register(L, "GetPlayerGold", GetPlayerGold);
    ...
    lua_register(L, "GetAllInteruptablePlayerAssetsWithCapability", GetAllInteruptablePlayerAssetsWithCapability);
    lua_register(L, "GetAllAssetsWithCapability", GetAllAssetsWithCapability);
    lua_register(L, "GetAllPlayerAssetsWithCapability", GetAllPlayerAssetsWithCapability);
    ...
    lua_register(L, "GetLocationsOfAssetsOfType",GetLocationsOfAssetsOfType);
    lua_register(L, "GetLocationsOfIdleAssetsOfType",GetLocationsOfIdleAssetsOfType);
    lua_register(L, "GetAllPlayerBuildingAssets", GetAllPlayerBuildingAssets);
    ...
    lua_register(L, "GetCanPlaceAssetOfSize", GetCanPlaceAssetOfSize);
    lua_register(L, "GetLocationsOfAllEnemyTypes",GetLocationsOfAllEnemyTypes);
	lua_register(L, "GetUpgradeStatus",GetUpgradeStatus);
    lua_register(L, "GetBuilder", GetBuilder);
    lua_register(L, "GetTownHallPos", GetTownHallPos);
    //Command Setters
    ...
    lua_register(L, "GetToggle", GetToggle);
    lua_register(L, "SwitchToggle", SwitchToggle);
}
...
//
//
//void CAIPlayer::CalculateCommand(SPlayerCommandRequest &command, std::string source){
//    if(READ){
//        LoadCommand(command, source);
//        return;
//    }
//
//=======
void CAIPlayer::CalculateCommand(SPlayerCommandRequest &command){
    ...
    if((DCycle % DDownSample) == 0){
        // printf("\n---CalculateCommand---%s, %s)\n", DLuaFile, ColorTypeToName(DPlayerData->Color()).c_str());
...
//        luaL_dofile(AIL, "./scripts/easy.lua");
        luaL_dofile(AIL, DLuaFile);
        //Set AI Pointer
        ...
        //Set Toggle
        lua_pushnumber(AIL, DToggles);
        lua_setglobal(AIL, "AIToggles");
    ...
    if (!DQueuedCommands.empty()){
        ...
        DQueuedCommands.pop();
        // printf("      Location: %d, %d\n", command.DTargetLocation.X(), command.DTargetLocation.Y());
    }
}
...
void CAIPlayer::CleanCommand(SPlayerCommandRequest &command){
...
//bool popLine(std::string& input, std::string& output){
//	if(input.empty()) return false;
//	
//	output = input.substr(0,input.find("\n"));
//	input = input.substr(input.find("\n") + 1);
//	if(output.front() == '#') popLine(input,output);
//	return true;
//	
//}

bool CAIPlayer::LoadCommand(SPlayerCommandRequest &command, std::string  source){
	...
	//PlayerColor off for the time being
	//Pop extra color value off
	popLine(source,Value);

    // load action
    if(!popLine(source,Value)){
        //PrintError("Failed to load action.\n");
        return false;
    }
	// std::cout << "Action: " << Value << std::endl;
    command.DAction = static_cast<EAssetCapabilityType>(stoi(Value));
    ...
    for(int i = 0; i < ActorCount; i++){
        ...
        // std::cout << "AssetID: " <<Value << std::endl;
    ...
    command.DActors = SelectedPlayerAssets;
    ...
    command.DTargetColor = static_cast<EPlayerColor>(stoi(Value));
    ...
    command.DTargetType = static_cast<EAssetType>(stoi(Value));
	// std::cout << Value << std::endl;
    ...
    command.DTargetLocation.X(stoi(Value));
	// std::cout << Value << std::endl;
    ...
    command.DTargetLocation.Y(stoi(Value));
	// std::cout << Value << std::endl;
