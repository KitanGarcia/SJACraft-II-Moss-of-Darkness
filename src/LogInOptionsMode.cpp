...
void CLogInOptionsMode::LogInOptionsUpdateButtonCallback(std::shared_ptr< CApplicationData > context){
	context->DServerUsername = DLogInOptionsModePointer->DEditText[0];
	context->DMultiplayerClient->login([context](bool status)
			{
			if(status) {
			context->DGameSessionType = CApplicationData::EGameSessionType::gstMultiPlayerServerHost;
	context->DUserInfo = context->DMultiplayerClient->getInfo();
	std::cout << "got user info\n";
	context->DMultiplayerClient->getGameInfo();
	std::cout << "got game info\n";
			context->ChangeApplicationMode(CGameSelectionMode::Instance());
			return status;
			}
			return false;
			}, context->DServerUsername,DHiddenPassword);
...
void CLogInOptionsMode::InitializeChange(std::shared_ptr< CApplicationData > context){
    DHiddenPassword = context->DServerPassword;
    DEditSelected = -1;
    ...
    context->DMultiplayerClient.reset(new CMultiplayerClient(context, std::string("104.236.151.124"), std::string("55107")));
	context->DMultiplayerClient->start_client_thread();
}
