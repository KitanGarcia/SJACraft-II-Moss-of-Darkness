...
*
* @brief This class maintains a players assets built on the map. It is derived from the class TerrainMap.
* The class contains a static list of shared pointers to other AssetDecoratedMaps. It also contains
* functions for loading maps, adding and removing player assets, and asset placement on the map.
*
* @author Jade
*
* @version 9.0
...
std::vector< std::shared_ptr< CAssetDecoratedMap > > CAssetDecoratedMap::DBackupMaps;
...
bool CAssetDecoratedMap::CanPlaceAsset(const CTilePosition &pos, int size, std::shared_ptr< CPlayerAsset > ignoreasset){
    int RightX, BottomY;
    bool isGoldVein = false;

    std::shared_ptr< CApplicationData > tempData = CApplicationData::Instance("");
    if(tempData->DPlayerCommands[to_underlying(ignoreasset->Color())].DAction == EAssetCapabilityType::BuildGoldMine || tempData->DCurrentAssetCapability == EAssetCapabilityType::BuildGoldMine){
        for(auto Asset : DAssets){
            if(EAssetType::GoldVein == Asset->Type()){
                if(Asset->TilePosition() == pos){
                    PrintDebug(DEBUG_LOW, "can place asset success gold mine\n");
                    isGoldVein = true;
                    return true;
                }
            }
    ...
    for(int YOff = 0; YOff < size; YOff++){
        for(int XOff = 0; XOff < size; XOff++){
            ...
            if(!CanPlaceOn(TileTerrainType) && !isGoldVein){
                return false;
            }
        }
    }
...
/**
* Remove lumber from a given tile position and update 12/3/17
*
...
void CAssetDecoratedMap::RemoveLumber(const CTilePosition &pos, const CTilePosition &from, int amount, bool add){
    int Index = 0;
    ...
    if(Index && (0xF != Index)){
        ...
        PrintDebug(DEBUG_LOW, "index chosen %d\n", Index);
        switch(Index){
            case 0: DLumberAvailable[pos.Y()][pos.X()] -= amount;
                    ...
                    if(add){
                        ChangeTerrainTilePartial(pos.X(), pos.Y(), 0xF);
                    }
                    break;
            case 1: DLumberAvailable[pos.Y()][pos.X()+1] -= amount;
                    ...
                    if(add){
                        ChangeTerrainTilePartial(pos.X()+1, pos.Y(), 0xF);
                    }
                    break;
            case 2: DLumberAvailable[pos.Y()+1][pos.X()] -= amount;
                    ...
                    if(add){
                        ChangeTerrainTilePartial(pos.X(), pos.Y()+1, 0xF);
                    }
                    break;
            case 3: DLumberAvailable[pos.Y()+1][pos.X()+1] -= amount;
                    ...
                    if(add){
                        ChangeTerrainTilePartial(pos.X()+1, pos.Y()+1, 0xF);
                    }
                    break;
        }
    }
...
/**
* Remove stone from a given tile position and update 12/3/17
*
...
/**
* Grow a tree back from the stumps, pass it x and y with borders
*
...
bool CAssetDecoratedMap::GrowTree(int x, int y){
    DMap[y][x] = ETileType::Forest;

    DLumberAvailable[y][x] = 400;
    ChangeTerrainTilePartial(x,y,0xF);
    DLumberAvailable[y][x-1] = 400;
    ChangeTerrainTilePartial(x-1,y,0xF);
    DLumberAvailable[y-1][x] = 400;
    ChangeTerrainTilePartial(x,y-1,0xF);
    DLumberAvailable[y-1][x-1] = 400;
    ChangeTerrainTilePartial(x-1,y-1,0xF);

    return true;
}

/**
* Process rubble for destroyed walls
*
* @param[in] pos The Tile position of the wall to be replaced
*
* @return void
*
*/

void CAssetDecoratedMap::CreateRubble(CTilePosition pos) {
    int XPos = pos.X();
    int YPos = pos.Y();
    DTerrainMap[YPos][XPos] = ETerrainTileType::Rock;
    DStoneAvailable[pos.Y()][pos.X()] = 0;
    ChangeTerrainTilePartial(pos.X(), pos.Y(), 0);
    DStoneAvailable[pos.Y()][pos.X()+1] = 0;
    ChangeTerrainTilePartial(pos.X()+1, pos.Y(), 0);
...
*
* @return true if loaded successfully or not
*
*/
...
/**
* Initialize the growth map 
*
* @param[in] Nothing
*
* @return the created growth map
*
*/

std::map< std::pair<int,int>, int> CAssetDecoratedMap::InitGrowthMap(){
    std::map< std::pair<int,int>, int> GrowthMap;

    // set up growth map
    for(int RowIndex = 0; RowIndex < DMap.size(); RowIndex++){
        for(int ColIndex = 0; ColIndex < DMap[RowIndex].size(); ColIndex++){
            if(CTerrainMap::ETileType::Forest == DMap[RowIndex][ColIndex]){
                PrintDebug(DEBUG_LOW, "forest at coord x=%d, y=%d, map size=%d\n", ColIndex, RowIndex, DMap.size());
                GrowthMap.insert( std::pair< std::pair<int,int>, int >(std::make_pair(ColIndex,RowIndex), 0));
            }
        }
    }
...
/**
* Get function, return the list of starting assets DAssetInitializationList
*
* @return the list of starting assets
*
*/
...
/**
* Get function, return the list of starting resources DResourceInitializationList
*
...
/**
* Update the visibility map based on the resource map
*
    ...
    while(SearchQueue.size()){
        ...
        for(int Index = 0; Index < (sizeof(SearchXOffsets)/sizeof(int)); Index++){
            ...
            if(SEARCH_STATUS_UNVISITED == DSearchMap[TempSearch.DY][TempSearch.DX]){
                ...
                if(type == CurTileType){
                    if(ETileType::Stump == type){
                        if(CApplicationData::Instance("")->GetGameModel()->NoAdolescent(TempSearch.DX, TempSearch.DY)){
...
/**
* Save map data to a output file stream
*
...
void CAssetDecoratedMap::SaveMap(std::ofstream& save) const{
    // Save AssetDecoratedMap
    CTerrainMap::SaveMap(save);
}

/**
* Replace an old map with a new map while backing up the old map
*
* @param[in] NewMap new map to replace the old map
* @param[in] Index index of the map to be replaced
*
* @return void
*
*/

void CAssetDecoratedMap::ReplaceMap(std::shared_ptr < CAssetDecoratedMap > NewMap, int index){
    auto OldMap = CAssetDecoratedMap::GetMap(index);

    // make a copy of the old map
    auto CopyMap = std::make_shared<CAssetDecoratedMap>(*OldMap);

    // push old map to backup maps
    DBackupMaps.push_back(CopyMap);

    // change old map with new map
    DAllMaps[index]->DPartials = NewMap->DPartials;
    DAllMaps[index]->DMap = NewMap->DMap;
    DAllMaps[index]->DMapIndices = NewMap->DMapIndices;
}
...
void CAssetDecoratedMap::ResetMap(){
    auto OldMap = DBackupMaps.back();

    // find index of the map to be replaces
    int index = CAssetDecoratedMap::FindMapIndex(OldMap->MapName());

    DAllMaps[index]->DPartials = OldMap->DPartials;
    DAllMaps[index]->DMap = OldMap->DMap;
    DAllMaps[index]->DMapIndices = OldMap->DMapIndices;

    // clear backup
    DBackupMaps.pop_back();
}
