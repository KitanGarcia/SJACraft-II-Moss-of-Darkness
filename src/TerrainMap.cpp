...
* @class TerrainMap
*
* @brief This class maintains the tile attributes of the map. The map is represented by two 2D vectors, 
* one is overlayed on top of the other. The ETerrainTileType (DTerrainMap) is the bottom layer and the 
* ETileType (DMap) is the top layer. DTerrainMap is used to determine what tiles make up DMap and  
* DMap is the map that is displayed. The DPartials map is used to keep track of how much lumber or stone
* can be acquired.
*
* @author Jade
*
            ...
            if(DRendered){
                ...
                if((0 < XPos)&&(0 < YPos)){
                    if((YPos + 1 < DMap.size())&&(XPos + 1 < DMap[YPos].size())){
                        ...
                        PrintDebug(DEBUG_LOW, "tile type is %d\n", static_cast<int>(Type));
                        DMap[YPos][XPos] = Type;
...
/**
* Counts the number of adjacent forest tiles, pass it x and y with border
*
* @param[in] x The x coordinate
* @param[in] y The y coordinate
*
* @return int count, number of tiles
*
*/

int CTerrainMap::CountAdjacentForest(int x, int y){
    int count = 0;

    if(((y-1) >= 0) && (ETileType::Forest == DMap[y-1][x])){
        count++;
    }
    if(((x+1) < DMap[0].size()) && (ETileType::Forest == DMap[y][x+1])) {
        count++;
    }
    if(((y+1) < DMap.size()) && (ETileType::Forest == DMap[y+1][x])){
        count++;
    }
    if(((x-1) >= 0) && (ETileType::Forest == DMap[y][x-1])){
        count++;
    }
...
*
* @return void, but the two parameters type and index are passed
...
* TerrainMap and adds a water tile border around the map
*
* @return void
*
*/
    ...
    for(int YPos = 0; YPos < DMap.size(); YPos++){
        if((0 == YPos)||(DMap.size() - 1 == YPos)){
            for(int XPos = 0; XPos < DTerrainMap[0].size() + 1; XPos++){
                DMap[YPos].push_back(ETileType::ShallowWater);
...
/**
* Saves terrain map to an output file stream
*
...
void CTerrainMap::SaveMap(std::ofstream& save) const{
    std::string TempString;
    std::vector< std::string > Tokens;
    int MapWidth, MapHeight;
    bool ReturnStatus = false;
    ...
    save << "#Map Name\n";
    save << DMapName << std::endl;

    save << "# Map Dimensions W x H\n";
    save << Width() << " " << Height() << "\n";

    save << "# Map Partial Bits\n";

    for(int Index = 0; Index < DTerrainMap.size(); Index++){
        for(int Inner = 0; Inner < DTerrainMap[0].size(); Inner++){
            if((0 <= DPartials[Index][Inner])&&(9 >= DPartials[Index][Inner])){
                save << char(DPartials[Index][Inner] + '0');
            }
            else if(('A' <= DPartials[Index][Inner] - 0x0A + 'A')&&('F' >= DPartials[Index][Inner] - 0x0A + 'A')){
                save << char(DPartials[Index][Inner] - 0x0A + 'A');
            }
        }
        save << "\n";
    }
    ...
    for(int Index = 0; Index < DTerrainMap.size() + 1; Index++){
        for(int Inner = 0; Inner < DTerrainMap[0].size() + 1; Inner++){
            save << char(to_underlying(DMap[Index][Inner]) + 'A');
        }
        save << "\n";
    }

    save << "# Map Indices Bits\n";

    for(int Index = 0; Index < DTerrainMap.size() + 1; Index++){
        for(int Inner = 0; Inner < DTerrainMap[0].size() + 1; Inner++){
            save << char(DMapIndices[Index][Inner] + 'A');
        }
        save << "\n";
    }

    // not sure if the following need to be saved

    // The following do not need to be saved since they will be initialized
    // according to the partial map and terrain map in RenderTerrain() which is
    // called in CGameModel()
    // DMapIndices
}

/**
* Restores terrain map data from a Data Source
*
* @param[in] source Data source file to read from
*
* @return True if map is restored without errors
*
*/

bool CTerrainMap::RestoreMap(std::shared_ptr< CDataSource > source){
    CCommentSkipLineDataSource LineSource(source, '#');
    std::string TempString;
    std::vector< std::string > Tokens;
    int MapWidth, MapHeight;
    ...
    try{
        ...
        DPartials.resize(MapHeight+1);
        for(int Index = 0; Index < MapHeight+1; Index++){
            DPartials[Index].resize(MapWidth+1);
            for(int Inner = 0; Inner < MapWidth+1; Inner++){
                if(('0' <= StringMap[Index][Inner])&&('9' >= StringMap[Index][Inner])){
                    DPartials[Index][Inner] = StringMap[Index][Inner] - '0';
        ...
        StringMap.clear();
        while(StringMap.size() < MapHeight + 2){
            if(!LineSource.Read(TempString)){
                goto LoadMapExit;
            }
            StringMap.push_back(TempString);
            if(MapWidth + 2 > StringMap.back().length()){
                goto LoadMapExit;
            }
        }
        if(MapHeight + 2 > StringMap.size()){
            goto LoadMapExit;
        }
        DMap.resize(MapHeight+2);
        for(int Index = 0; Index < MapHeight+2; Index++){
            DMap[Index].resize(MapWidth+2);
            for(int Inner = 0; Inner < MapWidth+2; Inner++){
                    DMap[Index][Inner] = static_cast<ETileType>(StringMap[Index][Inner] - 'A');
            }
        }
        ...
        DMapIndices.resize(MapHeight+2);
        for(int Index = 0; Index < MapHeight+2; Index++){
            DMapIndices[Index].resize(MapWidth+2);
            for(int Inner = 0; Inner < MapWidth+2; Inner++){
                    DMapIndices[Index][Inner] = StringMap[Index][Inner] - 'A';
            }
        }
