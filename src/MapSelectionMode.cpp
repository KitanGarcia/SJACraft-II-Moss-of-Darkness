...
/**
* Constructor for CMapSelectionMode
*
...
void CMapSelectionMode::SelectMapButtonCallback(std::shared_ptr< CApplicationData > context){
    context->DMapConfirmed = true;
    ...
    for(int Index = 0; Index < to_underlying(EPlayerColor::Max); Index++){
        ...
        if(Index){
            if(1 == Index){
                ...
                context->DLoadingPlayerTypes[Index] = (CApplicationData::gstMultiPlayerLANHost == context->DGameSessionType || CApplicationData::gstMultiPlayerServerHost == context->DGameSessionType) ?  CApplicationData::ptHuman : CApplicationData::ptAIEasy;
            }
        }
    }
    ...
    if(CApplicationData::gstMultiPlayerLANHost == context->DGameSessionType || CApplicationData::gstMultiPlayerServerHost == context->DGameSessionType){
        context->ChangeApplicationMode(CHostGameOptionsMode::Instance());
    }
    else{
        context->ChangeApplicationMode(CPlayerAIColorSelectMode::Instance());
...
void CMapSelectionMode::BackButtonCallback(std::shared_ptr< CApplicationData > context){
...
*
* @param[in] A pointer to CApplicationData.
*
* @return void
*
*/

void CMapSelectionMode::InitializeChange(std::shared_ptr< CApplicationData > context){
    ...
    context->DMapRenderer = std::make_shared< CMapRenderer >(std::make_shared< CMemoryDataSource >(context->DMapRendererConfigurationData), context->DTerrainTileset, context->DSelectedMap, context->DTreeTileset);
...
void CMapSelectionMode::Render(std::shared_ptr< CApplicationData > context){
    ...
    TextTop = BufferHeight - context->DBorderWidth - (context->DButtonRenderer->Height() * 9 / 4);
    if(ButtonXAlign){
    ...
    TextTop = BufferHeight - context->DBorderWidth - context->DButtonRenderer->Height();
...
/**
* Guarantees the existance of an instance of the map selection mode.
*
* @return shared_ptr to the instance of CMapSelectionMode.
*
*/

std::shared_ptr< CApplicationMode > CMapSelectionMode::Instance(){
    if(DMapSelectionModePointer == nullptr){
        DMapSelectionModePointer = std::make_shared< CMapSelectionMode >(SPrivateConstructorType());
    }
