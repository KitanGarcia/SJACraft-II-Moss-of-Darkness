...
static std::string DIconDescriptionString;
static std::string DMissingRequirements;
...
//    // DBattleStart = false;
//    DLeftClicked = false;
//    DPrevAction = EAssetCapabilityType::None;
//    DNextCommandCycle = -1;
//    // input.open("Input.txt");
//
//    if(READ)
//        source = std::make_shared< CFileDataSource >("CommandList.txt");
//    //else
//        //output.open("CommandList.txt");
//=======
    DTime = 0;
    DLeftClicked = false;
    DEditSelected = -1;
...
/**
* Load game map and play introductory sound
*
...
/**
*  Read input from the game to update 12/3/17
...
void CBattleMode::Input(std::shared_ptr< CApplicationData > context){
   // PrintDebug(DEBUG_LOW, "Started BattleMode::Input\n");
    ...
    for(auto Key : context->DPressedKeys){
        ...
        // handle unit grouping
            ...
            if(KeyLookup != context->DGroupHotKeyMap.end()){
                ...
                std::list<std::weak_ptr<CPlayerAsset> > UnitGroup;

                // take out enemy's units and buildings
                for(auto Asset : context->DSelectedPlayerAssets){
                    if(Asset.lock()->Color() == context->DPlayerColor && Asset.lock()->Speed() > 0){
                        UnitGroup.push_back(Asset);
                    }
                }
                ...
                // only reassign the list if any valid units are selected
                if(UnitGroup.size() && DLeftClicked){
                    context->DGroupHotKeyMap[Key].clear();

                    for(auto Asset : UnitGroup){
                        context->DGroupHotKeyMap[Key].push_back(Asset);
                    }
    ...
    for(auto Key : context->DReleasedKeys){
        ...
        if(SGUIKeyType::Return == Key){
            if(DEditSelected == -1)
                DEditSelected = 0;

            // if enter is pressed, send string to screen
            else{
                if(DEditText[DEditSelected].size() > 0){
                    SGameEvent TempEvent;
                    TempEvent.DType = EEventType::SendText;
                    TempEvent.DPlayer = context->DGameModel->Player(context->DPlayerColor);
                    context->DGameModel->Player(context->DPlayerColor)->AddGameEvent(TempEvent);
                    context->DNotificationRenderer->SendChat(DEditText[DEditSelected]);
                    if(context->DGameSessionType != CApplicationData::gstSinglePlayer)
                        context->DMultiplayerClient->sendChat(DEditText[DEditSelected]);
                    else
                        context->DGameModel->Player(context->DPlayerColor)->CheckCheats(DEditText[DEditSelected]);
                }
        ...
        if(0 == DEditSelected){
            DEditSelectedCharacter = DEditText[DEditSelected].size();
            SGUIKeyType TempKey;
            TempKey.DValue = Key;
        ...
        if(DEditSelected != 0 && context->DSelectedPlayerAssets.size()){
            bool CanMove = true;
            ...
            if(EAssetCapabilityType::BuildSimple == context->DCurrentAssetCapability){
                ...
                if(KeyLookup != context->DBuildHotKeyMap.end()){
                    ...
                    if(PlayerCapability){
                        ...
                        std::string dummyStr = "";
                        if(PlayerCapability->CanInitiate(ActorTarget, context->DGameModel->Player(context->DPlayerColor), dummyStr)){
                            SGameEvent TempEvent;
    ...
    if(CApplicationData::uictViewport == ComponentType){
        ...
        // else if(DBattleStart && context->DLeftClick){
            ...
            if((EAssetCapabilityType::None == context->DCurrentAssetCapability)||(EAssetCapabilityType::BuildSimple == context->DCurrentAssetCapability)){
            ...
            else if(EAssetCapabilityType::BuildWall == context->DCurrentAssetCapability){
                PrintDebug(DEBUG_LOW, "Entered BuildWall\n");
                if(context->DLeftDown){
                    context->DMouseDown = TempPosition;
                   // DWallPlacements.clear();
                    PrintDebug(DEBUG_LOW, "If Entered BuildWall\n");
                }
                else{

                    for(auto& name : {"Asset Name", "Health"})
                    PrintDebug(DEBUG_LOW, "Else Entered BuildWall\n");
                    SRectangle TempRectangle;
                    EPlayerColor SearchColor = context->DPlayerColor;
                    ...
                    CPixelType PixelType = CPixelType::GetPixelType(context->DViewportTypeSurface, DWallPlacements[0].GetDXPixel(), DWallPlacements[0].GetDYPixel());
                    context->DPlayerCommands[to_underlying(context->DPlayerColor)].DAction = context->DCurrentAssetCapability;
                    context->DPlayerCommands[to_underlying(context->DPlayerColor)].DActors = context->DSelectedPlayerAssets;
                    context->DPlayerCommands[to_underlying(context->DPlayerColor)].DTargetColor = PixelType.Color();
                    context->DPlayerCommands[to_underlying(context->DPlayerColor)].DTargetType = PixelType.AssetType();
                    context->DPlayerCommands[to_underlying(context->DPlayerColor)].DTargetLocation = CPixelPosition(DWallPlacements[0].GetDXPixel(), DWallPlacements[0].GetDYPixel());

                    for(std::vector< CTilePosition >::iterator it = DWallPlacements.begin()++; it != DWallPlacements.end(); it++){
                        CPixelType PixelType = CPixelType::GetPixelType(context->DViewportTypeSurface, it->GetDXPixel(), it->GetDYPixel());
                        SPlayerCommandRequest request;
                        request.DAction = context->DCurrentAssetCapability;
                        request.DActors = context->DSelectedPlayerAssets;
                        request.DTargetColor = PixelType.Color();
                        request.DTargetType = PixelType.AssetType();
                        request.DTargetLocation = CPixelPosition(it->GetDXPixel(), it->GetDYPixel());
                        DBufferedWallCommands.push_back(request);
                    }
              ...
              // context->DCurrentAssetCapability = EAssetCapabilityType::None;
                PrintDebug(DEBUG_LOW, "Exited BuildWall\n");
            }
            else{
                ...
                // Left button is held down
                    ...
                    context->DMouseDown = TempPosition;
                    // check wall
                    if(context->DCurrentAssetCapability == EAssetCapabilityType::BuildWall){
                        // not sure if this is useful
                    }
                }
            }
        }
        else if(!context->DLeftDown){
            DWallPlacements.clear();
        }
    }
        ...
        bool canMove = true;
        context->DDescriptionState = context->DLeftDown ? CButtonRenderer::EButtonState::Pressed : CButtonRenderer::EButtonState::Hover;
        ...
        if(context->DLeftClick && !context->DLeftDown){
            ...
            if(PlayerCapability){
                ...
                if((CPlayerCapability::ETargetType::None == PlayerCapability->TargetType())||(CPlayerCapability::ETargetType::Player == PlayerCapability->TargetType())){
                    ...
                    if(CapabilityType == EAssetCapabilityType::Shelter){
                        // context->DCurrentAssetCapability = EAssetCapabilityType::Shelter;

                        std::shared_ptr< CPlayerAsset > Asset = context->DSelectedPlayerAssets.front().lock();
                        std::shared_ptr< CPlayerAsset > NewTarget = context->DGameModel->Player(context->DPlayerColor)->FindNearestShelter(Asset->Position(), {EAssetType::TownHall, EAssetType::Keep, EAssetType::Castle}).lock();
                        if(PlayerCapability->CanApply(Asset, context->DGameModel->Player(context->DPlayerColor), NewTarget)){
                            ...
                            context->DPlayerCommands[to_underlying(context->DPlayerColor)].DAction = CapabilityType;
                            context->DPlayerCommands[to_underlying(context->DPlayerColor)].DActors = context->DSelectedPlayerAssets;
                            context->DPlayerCommands[to_underlying(context->DPlayerColor)].DTargetColor = context->DPlayerColor;
                            // context->DPlayerCommands[to_underlying(context->DPlayerColor)].DTargetColor = EPlayerColor::None;
                            context->DPlayerCommands[to_underlying(context->DPlayerColor)].DTargetType = NewTarget->Type();
                            // context->DPlayerCommands[to_underlying(context->DPlayerColor)].DTargetType = EAssetType::None;
                            context->DPlayerCommands[to_underlying(context->DPlayerColor)].DTargetLocation = NewTarget->Position();
                        }
        ...
        else if(context->DDescriptionState == CButtonRenderer::EButtonState::Hover){
            EAssetCapabilityType DescriptionType = context->DUnitActionRenderer->Selection(context->ScreenToUnitAction(CPixelPosition(CurrentX, CurrentY)));
            DIconDescriptionString = CPlayerCapability::TypeToName(DescriptionType);

            if(context->DSelectedPlayerAssets.size()){
                auto ActorTarget = context->DSelectedPlayerAssets.front().lock();
                auto playerdata = context->DGameModel->Player(context->DPlayerColor);
                auto Capability = CPlayerCapability::FindCapability(DescriptionType);

                if(Capability){
                    if(!Capability->CanInitiate(ActorTarget, playerdata, DMissingRequirements)){
                    }
                }
            }
        }
        ...
        context->DMenuButtonState = context->DLeftClick && !context->DLeftDown ? CButtonRenderer::EButtonState::Pressed : CButtonRenderer::EButtonState::Hover;
    ...
    if(!Panning){
        ...
        DIconDescriptionString = "";
    }

    //if(to_underlying(DPrevAction) != to_underlying(context->DPlayerCommands[to_underlying(context->DPlayerColor)].DAction))
        SaveCommand(context);

    DPrevAction = context->DPlayerCommands[to_underlying(context->DPlayerColor)].DAction;
...
/**
* Calculate current player assets and process input values
*
...
void CBattleMode::Calculate(std::shared_ptr< CApplicationData > context){
    ...
    std::vector< SAssetCommand > Commands;
    std::weak_ptr< CPlayerAsset> tempWeak;
    if(!context->DSelectedPlayerAssets.empty())
        tempWeak = context->DSelectedPlayerAssets.front();
    std::shared_ptr< CPlayerAsset > tempAsset;

    if(tempAsset = tempWeak.lock()) {
      //  PrintDebug(DEBUG_LOW, "Started BattleMode Calculate Type %d ActionCount = %d\n", (int) tempAsset->Type(), tempAsset->CommandCount());
        Commands = tempAsset->GetCommands();
        for(auto& com : Commands){
            PrintDebug(DEBUG_LOW, "BattleMode Start Calculate Count %d Type %d Action %d Capability %d\n", tempAsset->CommandCount(), (int) tempAsset->Type(), (int)com.DAction, (int) com.DCapability);
    ...
    //Command is full command sent over network
    ...
    for(int Index = 1; Index < to_underlying(EPlayerColor::Max); Index++){
        ...
        if(context->DGameModel->Player(static_cast<EPlayerColor>(Index))->IsAlive() && context->DGameModel->Player(static_cast<EPlayerColor>(Index))->IsAI()){
            ...
            if(context->DGameSessionType != CApplicationData::gstSinglePlayer){
                while(Value.empty()){
                    Command = context->DMultiplayerClient->getCommand();
                    popLine(Command,Value);//Pop Game Cycle
                }

                while(std::stoi(Value) != context->DGameModel->GameCycle()){
                    Command = context->DMultiplayerClient->getCommand();
                    popLine(Command,Value);//Pop Game Cycle
                }
                context->DAIPlayers[Index]->LoadCommand(context->DPlayerCommands[Index], Command);
  ...
  //}
    ...
    if ((context->DGameSessionType != CApplicationData::gstSinglePlayer && PlayerLeft == 1) || (context->DGameSessionType == CApplicationData::gstSinglePlayer && DBattleOver)){
        DBattleOver = false;
        if(context->DGameSessionType != CApplicationData::gstSinglePlayer){
            context->DMultiplayerClient->close();
            context->ChangeApplicationMode(CEndOfBattleMode::Instance());
        }
        else if ((!DForcedEnd && !AIAlive) || DForcedEnd)
            context->ChangeApplicationMode(CEndOfBattleMode::Instance());
    }

    for(std::vector< SPlayerCommandRequest >::reverse_iterator rit = DBufferedWallCommands.rbegin(); rit != DBufferedWallCommands.rend(); rit++){
        if(EAssetCapabilityType::None != rit->DAction){
            auto PlayerCapability = CPlayerCapability::FindCapability(rit->DAction);
            if(PlayerCapability){
                std::shared_ptr< CPlayerAsset > NewTarget;

                for(auto &WeakActor : rit->DActors){
                    if(auto Actor = WeakActor.lock()){
                        std::shared_ptr< CPlayerAsset > NewTarget;

                        if((CPlayerCapability::ETargetType::None != PlayerCapability->TargetType())&&(CPlayerCapability::ETargetType::Player != PlayerCapability->TargetType())){
                            if(EAssetType::None == rit->DTargetType){
                                NewTarget = context->DGameModel->Player(context->DPlayerColor)->CreateMarker(rit->DTargetLocation, true);
                            }
                            else{
                                NewTarget = context->DGameModel->Player(rit->DTargetColor)->SelectAsset(rit->DTargetLocation, rit->DTargetType).lock();
                            }
                        }
                        ...
                        Command.DCapability = EAssetCapabilityType::BuildWall;
                        Command.DAssetTarget = NewTarget;
                        Command.DAction = EAssetAction::Capability;
                        NewActor->PushCommand(Command);
                    }
                }
            ...
            if(PlayerCapability){
                ...
                if((CPlayerCapability::ETargetType::None != PlayerCapability->TargetType())&&(CPlayerCapability::ETargetType::Player != PlayerCapability->TargetType())){
                    if(EAssetType::None == context->DPlayerCommands[Index].DTargetType){
                      ...
                      PrintDebug(DEBUG_LOW, "Target Description: COLOR: %d,   LOCATION: (%d, %d), TYPE: %d", context->DPlayerCommands[Index].DTargetColor, context->DPlayerCommands[Index].DTargetLocation.X(), context->DPlayerCommands[Index].DTargetLocation.Y());
                ...
                if(!NewTarget && PlayerCapability->AssetCapabilityType() == EAssetCapabilityType::Attack) continue;
    ...
    if(tempAsset = tempWeak.lock()) {
        ...
        Commands = tempAsset->GetCommands();
        for(auto& com : Commands){
            PrintDebug(DEBUG_LOW, "BattleMode End Calculate Count = %d Type %d Action %d Capability %d\n", tempAsset->CommandCount(), (int) tempAsset->Type(), (int)com.DAction, (int) com.DCapability);
...
void CBattleMode::Render(std::shared_ptr< CApplicationData > context){
    ...
    while(!chats.empty()){
        ...
        TempEvent.DPlayer = context->DGameModel->Player(context->DPlayerColor);
        context->DGameModel->Player(context->DPlayerColor)->AddGameEvent(TempEvent);
        context->DNotificationRenderer->SendChat(chats.front());
    ...
    IconDescriptionW = context->DIconDescriptionSurface->Width();
    IconDescriptionH = context->DIconDescriptionSurface->Height();
    NotificationW = context->DNotificationSurface->Width();
    NotificationH = context->DNotificationSurface->Height();

    if(context->DLeftDown && 0 < context->DMouseDown.X()){
        TempPosition = CPixelPosition(context->ScreenToDetailedMap(CPixelPosition(CurrentX, CurrentY)));
        TempRectangle.DXPosition = std::min(context->DMouseDown.X(), TempPosition.X());
        TempRectangle.DYPosition = std::min(context->DMouseDown.Y(), TempPosition.Y());
        TempRectangle.DWidth = std::max(context->DMouseDown.X(), TempPosition.X()) - TempRectangle.DXPosition;
        TempRectangle.DHeight = std::max(context->DMouseDown.Y(), TempPosition.Y()) - TempRectangle.DYPosition;
     ...
     PrintDebug(DEBUG_LOW, "TempRectangle: X %d Y %d Width %d Height %d\n", TempRectangle.DXPosition, TempRectangle.DYPosition, TempRectangle.DWidth, TempRectangle.DHeight);

    // Clear wall placements when not building a wall or not holding down the
    // the left mouse when trying to build a wall
    if(context->DCurrentAssetCapability != EAssetCapabilityType::BuildWall){
        DWallPlacements.clear();
    }
    else if(context->DCurrentAssetCapability == EAssetCapabilityType::BuildWall && !context->DLeftDown){
        DWallPlacements.clear();
    }
    // Get coordinates of new tile and create a tile position to insert into map
    int TempX = (context->DMouseDown.X() < TempPosition.X()) ? TempRectangle.DXPosition + TempRectangle.DWidth : TempRectangle.DXPosition;
    int TempY = (context->DMouseDown.Y() < TempPosition.Y()) ? TempRectangle.DYPosition + TempRectangle.DHeight: TempRectangle.DYPosition;
    CTilePosition TempTilePosition = CTilePosition();
    TempTilePosition.SetXFromPixel(TempX);
    TempTilePosition.SetYFromPixel(TempY);

    DWallPlacements.push_back(TempTilePosition);
    DWallPlacements.erase(std::unique(DWallPlacements.begin(), DWallPlacements.end(), CTilePosition::Equals), DWallPlacements.end());

    if(context->DCurrentAssetCapability == EAssetCapabilityType::BuildWall){
        TempRectangle = SRectangle({0,0,0,0});
    }

    PrintDebug(DEBUG_LOW, "WallTilePosition: X %d Y %d\n", TempTilePosition.X(), TempTilePosition.Y());
    PrintDebug(DEBUG_LOW, "Print DWallPlacements (X,Y) Size %d : ", DWallPlacements.size());
    for(int i = 0; i < DWallPlacements.size(); i++) {
        PrintDebug(DEBUG_LOW, "(%d,%d) ", DWallPlacements[i].X(), DWallPlacements[i].Y());
    }
   // for(std::vector< CTilePosition >::iterator it = DWallPlacements.begin(); it != DWallPlacements.end(); ++it) {
     //   PrintDebug(DEBUG_LOW, "(%d,%d) ", it->X(), it->Y());
   // }
    ...
    context->DInnerBevel->DrawBevel(context->DWorkingBufferSurface, context->DIconDescriptionXOffset, context->DIconDescriptionYOffset, IconDescriptionW, IconDescriptionH);
    context->DIconDescriptionSurface->Draw(context->DWorkingBufferSurface, 0, 0, IconDescriptionW, IconDescriptionH, context->DIconDescriptionXOffset, context->DIconDescriptionYOffset);
    context->DIconDescriptionRenderer->DrawIconDescription(context->DIconDescriptionSurface, DIconDescriptionString, DMissingRequirements, context->DIconDescriptionXOffset, context->DIconDescriptionYOffset, IconDescriptionW, IconDescriptionH);
    context->DWorkingBufferSurface->Draw(context->DIconDescriptionSurface, context->DIconDescriptionXOffset, context->DIconDescriptionYOffset, -1, -1, 0, 0 );
    ...
    context->DViewportRenderer->DrawViewport(DWallPlacements, context->DViewportSurface, context->DViewportTypeSurface, SelectedAndMarkerAssets, TempRectangle, context->DCurrentAssetCapability);
    context->DMiniMapRenderer->DrawMiniMap(context->DMiniMapSurface);
    int ChatBoxH = 0;

    if(DEditSelected == 0){
        context->DChatBoxRenderer->Text(DEditText[DEditSelected], true);
        context->DChatBoxRenderer->DrawChatBox(context->DViewportSurface, 0, ViewHeight - context->DChatBoxRenderer->Height(), 0 == DEditSelected ? DEditSelectedCharacter : -1);
        ChatBoxH = context->DChatBoxRenderer->Height();
    }

    context->DNotificationRenderer->DrawNotification(context->DViewportSurface, context->DGameModel->Player(context->DPlayerColor)->GameEvents(), context->DViewportXOffset, ViewHeight - NotificationH - ChatBoxH, NotificationW, NotificationH);
...
void CBattleMode::SaveCommand(std::shared_ptr< CApplicationData > context){
    std::stringstream output;
    output << "#Game Cycle\n" << context->DGameModel->GameCycle() << "\n";
    output << "#PlayerColor\n" << to_underlying(context->DPlayerColor) << "\n";
    output << "#Action\n" << to_underlying(context->DPlayerCommands[to_underlying(context->DPlayerColor)].DAction) << "\n";
    output << "#Actors Count\n" << context->DPlayerCommands[to_underlying(context->DPlayerColor)].DActors.size() << "\n";
    output << "#AssetID\n";
    for(auto Actor : context->DPlayerCommands[to_underlying(context->DPlayerColor)].DActors){
        output << Actor.lock()->AssetID() << "\n";
    }

    // context->DPlayerCommands[to_underlying(context->DPlayerColor)].DTargetColor = EPlayerColor::None;
    output << "#TargetColor\n" << to_underlying(context->DPlayerCommands[to_underlying(context->DPlayerColor)].DTargetColor) << "\n";

    // context->DPlayerCommands[to_underlying(context->DPlayerColor)].DTargetType = EAssetType::None;
    output << "#TargetType\n" << to_underlying(context->DPlayerCommands[to_underlying(context->DPlayerColor)].DTargetType) << "\n";

    // context->DPlayerCommands[to_underlying(context->DPlayerColor)].DTargetLocation = ActorTarget->Position();
    // this is a CPixelPosition object
    output << "#TargetLocation\n" << context->DPlayerCommands[to_underlying(context->DPlayerColor)].DTargetLocation.X() << "\n";
    output << context->DPlayerCommands[to_underlying(context->DPlayerColor)].DTargetLocation.Y() << "\n";

    if(context->DGameSessionType != CApplicationData::gstSinglePlayer){
        context->DMultiplayerClient->sendCommand(output.str());
    }
}


/**
 * Load player commands from files
 *
 * @param[in] .
 *
 ...
 *
 */
bool CBattleMode::LoadCommand(std::shared_ptr< CApplicationData > context){
    std::string Value;
    ...
    if(!LineSource.Read(Value)){
        PrintError("Failed to load player color.\n");
        return false;
    }
    context->DPlayerColor = static_cast<EPlayerColor>(stoi(Value));

    // std::cout << "Color: " << Value << std::endl;

    // load action
    if(!LineSource.Read(Value)){
        PrintError("Failed to load action.\n");
        return false;
    }
    context->DPlayerCommands[to_underlying(context->DPlayerColor)].DAction = static_cast<EAssetCapabilityType>(stoi(Value));

    // std::cout << "Action: " << Value << std::endl;

    // load actor count
    if(!LineSource.Read(Value)){
        PrintError("Failed to load actor count.\n");
        return false;
    }
    int ActorCount = stoi(Value);

    // std::cout << "Actor count: " << Value << std::endl;

    std::list< std::weak_ptr< CPlayerAsset > > SelectedPlayerAssets;

    for(int i = 0; i < ActorCount; i++){
        // load asset IDs
        if(!LineSource.Read(Value)){
            PrintError("Failed to asset ID.\n");
            return false;
        }
        // std::cout << "ID: " << Value << std::endl;

        std::shared_ptr< CPlayerAsset > FoundAsset = FindAssetObj(std::stoi(Value));

        if(FoundAsset == nullptr)
            std::cout << "NOT FOUND!!!!!\n";

        SelectedPlayerAssets.push_back(FoundAsset);
    }

    context->DPlayerCommands[to_underlying(context->DPlayerColor)].DActors = SelectedPlayerAssets;

    // load target color
    if(!LineSource.Read(Value)){
        PrintError("Failed to load target color.\n");
        return false;
    }
    context->DPlayerCommands[to_underlying(context->DPlayerColor)].DTargetColor = static_cast<EPlayerColor>(stoi(Value));

    // std::cout << "Target color: " << Value << std::endl;

    // load target color
    if(!LineSource.Read(Value)){
        PrintError("Failed to load target type.\n");
        return false;
    }
    context->DPlayerCommands[to_underlying(context->DPlayerColor)].DTargetType = static_cast<EAssetType>(stoi(Value));

    // std::cout << "Target Type: " << Value << std::endl;

    // load target location
    if(!LineSource.Read(Value)){
        PrintError("Failed to load target X coordinate.\n");
        return false;
    }
    context->DPlayerCommands[to_underlying(context->DPlayerColor)].DTargetLocation.X(stoi(Value));
    // std::cout << "X: " << Value << std::endl;

    // load target location
    if(!LineSource.Read(Value)){
        PrintError("Failed to load target Y coordinate.\n");
        return false;
    }
    context->DPlayerCommands[to_underlying(context->DPlayerColor)].DTargetLocation.Y(stoi(Value));
