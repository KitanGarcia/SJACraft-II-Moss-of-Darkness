...
/**
*
* @class CUnitDescriptionRendere
*
* @brief This class is renders the Unit attribute values such as health, attack/defense values, speed, etc. The class is also
*        responsible for rendering the % completed for buildings. In addition, the notification and Icon descriptions are
*        rendered here. The icon descriptions show the button map of the currently hovered icon. Notifications are displayed
*        for a selected amount of time and dissapear after.
*
* @author Alex
*
* @version 9.0
...
#define HISTORY_LENGTH  9

std::unordered_map<std::string, std::string> CUnitDescriptionRenderer::DNametoKey({
            {"None", ""},
            {"BuildPeasant",    "[P]"},
            {"BuildFootman",    "[F]"},
            {"BuildKnight",     " "},
...
EventMap_t DEventMap = {
            {EEventType::None, "None"},
            {EEventType::WorkComplete, "Work Completed"},
            {EEventType::Selection, "Selection"},
            {EEventType::Acknowledge, "Acknowledge"},
            {EEventType::Ready, "Ready"},
            {EEventType::Death, "Death"},
            {EEventType::Attacked, "Attacked"},
            {EEventType::MissleFire, "MissleFire"},
            {EEventType::MissleHit, "MissleHit"},
            {EEventType::Harvest, "Harvest"},
            {EEventType::Quarry, "Quarry"},
            {EEventType::MeleeHit, "MeleeHit"},
            {EEventType::PlaceAction, "PlaceAction"},
            {EEventType::ButtonTick, "ButtonTick"},
            {EEventType::WinXGold, "WinXGold"},
            {EEventType::SpawnEnemies, "SpawnEnemies"},
            {EEventType::SendText, "SendText"},
            {EEventType::Max, "Max"}
};
 ...
 *
 * @param[in] bevel pointer to shading technique
 * @param[in] icons vector of tiles pictures
 * @param[in] fonts array of various sized fonts (i.e. small, medium, large, etc)
 * @param[in] color enumeration integer representing player color
 */
CUnitDescriptionRenderer::CUnitDescriptionRenderer(std::shared_ptr< CBevel > bevel, std::shared_ptr< CGraphicMulticolorTileset > icons, std::shared_ptr< CGraphicTileset > rangertrackingicon, std::array< std::shared_ptr< CFontTileset >, to_underlying(EFontSize::Max) > fonts, EPlayerColor color){
    ...
    DAssetIndices[to_underlying(EAssetType::Knight)] = DIconTileset->FindTile("knight");
    ...
    DResearchIndices[to_underlying(EAssetCapabilityType::BuildKnight)] = DIconTileset->FindTile("knight");
}

CUnitDescriptionRenderer::CUnitDescriptionRenderer(std::string str, std::shared_ptr< CBevel > bevel, std::shared_ptr< CGraphicMulticolorTileset > icons, std::array< std::shared_ptr< CFontTileset >, to_underlying(EFontSize::Max) > fonts, EPlayerColor color){
    int TextWidth, TextHeight;
    ...
    DNotificationHistory.resize(HISTORY_LENGTH);

    for(int Index = 0; Index < to_underlying(EFontSize::Max); Index++){
        DFonts[Index] = fonts[Index];

        DFontColorIndices[Index].resize(2);
        DFontColorIndices[Index][FG_COLOR] = DFonts[Index]->FindColor("gold");
        DFontColorIndices[Index][BG_COLOR] = DFonts[Index]->FindColor("black");
    }
    DFonts[to_underlying(EFontSize::Small)]->MeasureText("0123456789", TextWidth, TextHeight);
 ...
 * @return[in] string for AssetName
 */
...
/**
 * @brief Draws the icons currently selected in the middle viewport
 * param[in] pos current pixel position
 * @return The selected icon if any
 */
...
/**
 * @brief Dras the completion bar during building/unit construction
 * param[in] surface: The source surface on which to draw completion bar
 * param[in] percent: The percent competed out of 100
 */
...
/**
 * @brief Sets Minimum Width
 * Draws the attributed for a single asset or draws the tiles of multiple selected assets.
 * If only one asset is selected we get the detailed description of attributed such as health, damage, etc.
 * For multiple selected assets only the icon representing that asset is drawn in a list
 * param[in] surface: Source surface to draw on
 * param[in] selectionlist: The list of currently selected assets
 * @return integer x-coordinate dimension
 */
...
//                if(EPlayerColor::None != Asset->Color()){
                    ...
                    */
                    ...
                    TextTop += TextHeight;
 //               }
                if(DPlayerColor == Asset->Color() || Asset->Color() == EPlayerColor::None){
                   if(Asset->Speed()){
                        int TextLineHeight;
                        int UpgradeValue;
                        ...
                        if(EAssetAction::Construct == Asset->Action()){
                            ...
                            if(Command.DAssetTarget){
                                ...
                                */
                                ...
                                DBevel->DrawBevel(surface, HorizontalOffset, VerticalOffset, DIconTileset->TileWidth(), DIconTileset->TileHeight());
                                PrintDebug(DEBUG_LOW, "Command.DCapability: %d\n", (int)Command.DCapability);
                                if (EAssetCapabilityType::RangerTrackingUpgrade == Command.DCapability){
                                    DRangerTrackingIcon->DrawTile(surface, HorizontalOffset, VerticalOffset, 0);
                                }
                                else{
...
void CUnitDescriptionRenderer::DrawIconDescription(std::shared_ptr< CGraphicSurface > surface, std::string str, std::string str2, int xOffset, int yOffset, int width, int height){
    auto ResourceContext = surface->CreateResourceContext();
    ResourceContext->SetSourceRGB(0x000000);
    ResourceContext->Rectangle(0, 0, width, height);
    ResourceContext->Fill();

    DDisplayedWidth = width;
    DDisplayedHeight = height;

    if(!str.empty()){
        auto TempString = DNametoKey.find(str);
        str += TempString->second;

        if(str2.size()){
            str += " ";
            str += str2;
            DFontColorIndices[to_underlying(EFontSize::Small)][FG_COLOR] = DFonts[to_underlying(EFontSize::Small)]->FindColor("red");
        }
        else{
            DFontColorIndices[to_underlying(EFontSize::Small)][FG_COLOR] = DFonts[to_underlying(EFontSize::Small)]->FindColor("white");
        }
    }
    ...
    DFonts[to_underlying(EFontSize::Small)]->DrawTextWithShadow(surface,
                        0,
                        0,
                        DFontColorIndices[to_underlying(EFontSize::Small)][FG_COLOR], DFontColorIndices[to_underlying(EFontSize::Small)][BG_COLOR], 1, str);

}

void CUnitDescriptionRenderer::DrawNotification(std::shared_ptr< CGraphicSurface > surface, std::vector< SGameEvent > notification, int xOffset, int yOffset, int width, int height){

    // Create a new struct to push into vector
    kvNotifications newNotification;

    // Reduce the Timer counter for each element in the vector
    auto itr = DNotificationHistory.begin();

    while(itr != DNotificationHistory.end()){
        if(itr->Timer == 100){
            itr++;
            continue;
        }

        // Remove Event from vector if Timer hits 0, break to avoid memory issues
        if(itr->Timer == 0){
            itr = DNotificationHistory.erase(itr);
        }
        else{
            itr->Timer -= 1;
            itr++;
    ...
    // int TextHeight = 0;
    for(auto Event : notification){

        auto string = DEventMap.find(Event.DType);

        // only print in history if EventType is Ready, Work Completed, Attacked
        if((string->second != "Ready")&&(string->second != "Work Completed")&&(string->second != "Attacked")&& (string->second != "SendText")){
            continue;
        }

        // check for chat message
        if(Event.DType == EEventType::SendText){
            newNotification.Event = "Player: ";
            newNotification.Event += DChatStrings.front();
            newNotification.Timer = 100;
            newNotification.Color = Event.DPlayer->Color();
            newNotification.Name = "";

            // clear chat message string
            DChatStrings.erase(DChatStrings.begin());
        }
        else
            newNotification.Event = string->second;

        if(DNotificationHistory.size() < HISTORY_LENGTH){
                DNotificationHistory.push_back(newNotification);
        }
        else{
            DNotificationHistory.erase(DNotificationHistory.begin());
            DNotificationHistory.push_back(newNotification);
        }
    }// For_Events
    ...
    for(auto itr = DNotificationHistory.begin() ; itr != DNotificationHistory.end(); itr++){
        switch(itr->Color){
            case EPlayerColor::None:
                DFontColorIndices[to_underlying(EFontSize::Small)][FG_COLOR] = DFonts[to_underlying(EFontSize::Small)]->FindColor("gold");
                break;
            case EPlayerColor::Red:
                DFontColorIndices[to_underlying(EFontSize::Small)][FG_COLOR] = DFonts[to_underlying(EFontSize::Small)]->FindColor("red");
                break;
            case EPlayerColor::Blue:
                DFontColorIndices[to_underlying(EFontSize::Small)][FG_COLOR] = DFonts[to_underlying(EFontSize::Small)]->FindColor("blue");
                break;
            case EPlayerColor::Green:
                DFontColorIndices[to_underlying(EFontSize::Small)][FG_COLOR] = DFonts[to_underlying(EFontSize::Small)]->FindColor("green");
                break;
            case EPlayerColor::Purple:
                DFontColorIndices[to_underlying(EFontSize::Small)][FG_COLOR] = DFonts[to_underlying(EFontSize::Small)]->FindColor("purple");
                break;
            case EPlayerColor::Orange:
                DFontColorIndices[to_underlying(EFontSize::Small)][FG_COLOR] = DFonts[to_underlying(EFontSize::Small)]->FindColor("orange");
                break;
            case EPlayerColor::Yellow:
                DFontColorIndices[to_underlying(EFontSize::Small)][FG_COLOR] = DFonts[to_underlying(EFontSize::Small)]->FindColor("yellow");
                break;
            case EPlayerColor::Black:
                DFontColorIndices[to_underlying(EFontSize::Small)][FG_COLOR] = DFonts[to_underlying(EFontSize::Small)]->FindColor("black");
                break;
            case EPlayerColor::White:
                DFontColorIndices[to_underlying(EFontSize::Small)][FG_COLOR] = DFonts[to_underlying(EFontSize::Small)]->FindColor("while");
                break;
        }
        ...
        DFonts[to_underlying(EFontSize::Small)]->DrawTextWithShadow(surface,
                    0,
                    DTextHeight,
                    DFontColorIndices[to_underlying(EFontSize::Small)][FG_COLOR], DFontColorIndices[to_underlying(EFontSize::Small)][BG_COLOR], 1, (*itr).Event);
        DTextHeight += 15;
