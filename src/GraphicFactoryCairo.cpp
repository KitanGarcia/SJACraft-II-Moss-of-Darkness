...
/**
*
* @class CGraphicResourceContextCairo
*
* @brief This is the concrete class impementation for the graphics surface constructor. It is responsible for setting the type 
*        of surface rendered, the colors of the surface, its size (height/width). It has various components seperate from the 
*        complete representation of a surface such as draw lines, line types, bevel, selection rectangles, etc. In addition, 
*        it does bounds checking on the surface creation.
*
* @author Alex
*
* @version 9.0
...
void CGraphicSurfaceCairo::Draw(std::shared_ptr<CGraphicSurface> srcsurface, int dxpos, int dypos, int width, int height, int sxpos, int sypos, bool RangerInForest){
    ...
    //ResourceContext->SetSourceRGBA(0xAA000000);
    ...
    if(RangerInForest){
        //cairo_set_source_rgba(CairoResourceContext->DResourceContext, 0,0,0,0);
        //CairoResourceContext->DResourceContext->SetSourceRGBA(0xAA000000);
        cairo_set_operator(CairoResourceContext->DResourceContext, CAIRO_OPERATOR_XOR);
    }
