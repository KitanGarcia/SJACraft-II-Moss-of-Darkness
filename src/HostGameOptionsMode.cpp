...
void CHostGameOptionsMode::MapSelectionButtonCallback(std::shared_ptr< CApplicationData > context){
    context->DGameName = DHostGameOptionsModePointer->DEditText[0];
    context->DGamePassword = DHostGameOptionsModePointer->DEditText[1];

    if(context->DGameSessionType == CApplicationData::gstMultiPlayerLANHost){
        context->DLANPort = std::stoi(DHostGameOptionsModePointer->DEditText[2]);
    }
    else
        context->DServerPort = std::stoi(DHostGameOptionsModePointer->DEditText[2]);
...
void CHostGameOptionsMode::HostGameOptionsUpdateButtonCallback(std::shared_ptr< CApplicationData > context){
	context->DMultiplayerClient->hostGame([context](bool status)
	{
	if(status) {
	context->DMultiplayerClient->getPlayerInfo();
	context->DGameSessionType = CApplicationData::gstMultiPlayerServerHost;
	context->ChangeApplicationMode(CGameLobbyMode::Instance());
	return status;
	}
	return false;
	}, DHostGameOptionsModePointer->DEditText[3], context->DSelectedMap->PlayerCount(), DHiddenPassword, 0, DHostGameOptionsModePointer->DEditText[0]);
}
...
void CHostGameOptionsMode::InitializeChange(std::shared_ptr< CApplicationData > context){
    ...
    DEditText.clear();
    DEditText.push_back(context->DGameName);
    DEditText.push_back(context->DGamePassword);

    if(context->DGameSessionType == CApplicationData::gstMultiPlayerLANHost){
        DEditText.push_back(std::to_string(context->DLANPort));
    }
    else
        DEditText.push_back(std::to_string(context->DServerPort));
