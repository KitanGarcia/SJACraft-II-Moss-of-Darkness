 ...
 */
 void CEventHandler::RegisterFunctions (lua_State *L){
    ...
    lua_register(L, "GetPlayerUnits", GetPlayerUnits);
}
...
int CEventHandler::EndGame (lua_State *L){
    bool won = lua_toboolean(L, -2);
    bool forced = lua_toboolean(L, -1);
    CBattleMode::TriggeredEnd(won, forced);
    return 0;
}
...
int CEventHandler::GetPlayerUnits(lua_State *L){
    EPlayerColor targetColor = EPlayerColor::None;

    for (int i = 1; i < (int)EPlayerColor::Max; i++)
        if (!DGameModel->Player(static_cast<EPlayerColor>(i))->IsAI()){
            targetColor = static_cast<EPlayerColor>(i);
            // printf("Color %d\n", i);
            break;
        }
    ...
    lua_newtable(L);
    int index = 1;
    auto Assets = DGameModel->Map()->Assets();
    for(auto Asset : Assets){
        if(Asset->Color() == targetColor && Asset->Speed()){
...
int CEventHandler::GetUnitsOfColor(lua_State *L){
