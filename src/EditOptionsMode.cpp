...
/**
* Constructor that sets button hovered to false
*
*/
...
void CEditOptionsMode::Input(std::shared_ptr< CApplicationData > context){
    ...
    for(auto Key : context->DReleasedKeys){
        if(SGUIKeyType::Escape == Key){
        ...
        else if(SGUIKeyType::Tab == Key){
            DEditSelected = (DEditSelected + 1) % DEditText.size();

            if(DEditSelected >= 0 && DEditSelected < DEditText.size()){
                ClickedEdit = true;
            ...
            if((SGUIKeyType::Delete == Key)||(SGUIKeyType::BackSpace == Key)){
                ...
                if(TempKey.IsNumPad())
                    DEditText[DEditSelected] = DEditText[DEditSelected].substr(0,DEditSelectedCharacter) + std::string(1, (char)(Key - (SGUIKeyType::KeyPad0 - SGUIKeyType::Key0))) + DEditText[DEditSelected].substr(DEditSelectedCharacter,DEditText[DEditSelected].length()-DEditSelectedCharacter);
                else   
...
/**
* Calculate the inputed values for Options (Not implemented)
*
* @param[in] context a shared pointer to Application Data
*
* @return void
...
/**
* Render the graphics for Options Mode when editing
*
* @param[in] context a shared pointer to Application Data
*
* @return void
