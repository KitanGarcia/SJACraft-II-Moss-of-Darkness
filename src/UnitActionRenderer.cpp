...
*
* @brief This class renders the Action Icons for either buildings, peasants or army units. The icons are rendered in the lower
*        left corner corresponding to the activated abilities of that particular asset. It also has utility functions to check
*        size of icons to align them correctly.
*
* @author Alex
*
* @version 9.0
 ...
 *
 * @param[in] bevel pointer slope of shading to create 3D effect
 * @param[in] icons graphics pointer
 * @param[in] color player's color
 * @param[in] player game data specific to player
 */
CUnitActionRenderer::CUnitActionRenderer(std::shared_ptr< CBevel > bevel, std::shared_ptr< CGraphicTileset > icons, std::shared_ptr< CGraphicTileset > rangertrackingicon, std::shared_ptr< CGraphicTileset > peasantsheltericon, EPlayerColor color, std::shared_ptr< CPlayerData > player){
    DIconTileset = icons;
    DRangerTrackingIcon = rangertrackingicon;
    DPeasantShelterIcon = peasantsheltericon;
    DBevel = bevel;
    DPlayerData = player;
    DPlayerColor = color;
    ...
    DCommandIndices[to_underlying(EAssetCapabilityType::BuildGoldMine)] = DIconTileset->FindTile("gold-mine");
 ...
 * Only icons that are capable by the unit are rendered and the ones not currently available are rendered opaquely.
 *
 * @param[in] surface pointer to render image
 * @param[in] selectionlist list of all capabilities
 * @param[in] currentaction chosen action (i.e. move, build, attack, etc)
 */
void CUnitActionRenderer::DrawUnitAction(std::shared_ptr< CGraphicSurface > surface, const std::list< std::weak_ptr< CPlayerAsset > > &selectionlist, EAssetCapabilityType currentaction){
    ...
    if(EAssetCapabilityType::None == currentaction){
        if(Moveable){
            ...
            if(auto Asset = selectionlist.front().lock()){
                ...
                if(Asset->HasCapability(EAssetCapabilityType::Shelter)){
                    DDisplayedCommands[5] = EAssetCapabilityType::Shelter;
                }
            ...
            if(auto Asset = selectionlist.front().lock()){
                bool IsShelter = (Asset->Type() == EAssetType::TownHall || Asset->Type() == EAssetType::Castle || Asset->Type() == EAssetType::Keep) && Asset->HasAction(EAssetAction::StandGround);
                if((EAssetAction::Construct == Asset->Action())||(EAssetAction::Capability == Asset->Action()) || IsShelter){
                    DDisplayedCommands.back() = EAssetCapabilityType::Cancel;
                }
                else{
                    int Index = 0;
                    for(auto &Capability : Asset->Capabilities()){
                        if(Capability != EAssetCapabilityType::Shelter)
    ...
    for(auto IconType : DDisplayedCommands){
        if(EAssetCapabilityType::None != IconType){
            ...
            PrintDebug(DEBUG_LOW, "Icon Type = %d\n", (int)IconType);
            if (EAssetCapabilityType::RangerTrackingUpgrade == IconType){
                DRangerTrackingIcon->DrawTile(surface, XOffset, YOffset, 0);
            }
            else if (EAssetCapabilityType::Shelter == IconType){
                DPeasantShelterIcon->DrawTile(surface, XOffset, YOffset, 0);
            }
            else{
            ...
            if(PlayerCapability){
                std::string dummyStr = "";
                if(!PlayerCapability->CanInitiate(selectionlist.front().lock(), DPlayerData, dummyStr)){
