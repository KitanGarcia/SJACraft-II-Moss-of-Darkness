...
class CActivatedPlayerCapability{
    ...
    public:
        ...
        virtual void Save(std::ofstream& save) = 0;
};
...
class CPlayerCapability{
    ...
    public:
        ...
        virtual bool CanInitiate(std::shared_ptr< CPlayerAsset > actor, std::shared_ptr< CPlayerData > playerdata, std::string& message) = 0;
...
class CPlayerAssetType{
    ...
    public:
        ...
        void ChangeColor(EPlayerColor color){
            DColor = color;
        };
        ...
        std::vector< std::shared_ptr< CPlayerUpgrade > > GetUpgrades() const{
            return DAssetUpgrades;
        }

        bool FindUpgrade(std::string upgradename) {
            for(auto &upgrade : DAssetUpgrades){
...
class CPlayerAsset{
    ...
    public:
        ...
        ~CPlayerAsset();
        void PushPeasant(int peasant);
        void RemovePeasant(int peasant);
        int FindPeasant(int peasant);
        int PopPeasant();

        int PeasantSize(){
            return DPeasants.size();
        }
        ...
        int MoveRemainderX() const{
            return DMoveRemainderX;
        };

        int MoveRemainderX(int X){
            return DMoveRemainderX = X;
        };

        int MoveRemainderY() const{
            return DMoveRemainderY;
        };

        int MoveRemainderY(int Y){
            return DMoveRemainderY = Y;
        };
        ...
        int ReloadSteps() const{
            if((Type() == EAssetType::TownHall || Type() == EAssetType::Castle || Type() == EAssetType::Keep) && DPeasants.size() > 0)
                return DType->ReloadSteps() / DPeasants.size();
        ...
        int FreeSpace(int freeSpace){
            return DFreeSpace = freeSpace;
        }

        void SavePeasants(std::ofstream& save) const;
        void SaveCommands(std::ofstream& save, EPlayerColor PlayerColor) const;
        void LoadCommands(std::shared_ptr< CDataSource > source, std::array< std::shared_ptr< CPlayerData >, to_underlying(EPlayerColor::Max)> DPlayers);
